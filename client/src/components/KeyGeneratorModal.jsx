...
function KeyGeneratorModal({ isOpen, onClose, onKeyGenerated }) {
  ...
  const handleGenerate = React.useCallback((modeOverride) => {
    try {
  ...
  const handlePresetChange = React.useCallback((presetName) => {
  ...
  const handleUseKey = React.useCallback(() => {
    if (!generatedKey) {
  ...
  }, [generatedKey, onKeyGenerated, onClose]);
  ...
  React.useEffect(() => {
    if (isOpen) {
      console.log('Modal opened, generating key with mode:', mode);
      handleGenerate();
      setHistory(KeyGenerator.getHistory());
    }
  }, [isOpen, handleGenerate, mode]);
